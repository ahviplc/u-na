package com.lc.una.utils.tools.upload;
...
//                }
				...
				try {
					...
					StaticLog.error("文件没有发现" + e);
				} catch (IOException e) {
					StaticLog.error("文件读取失败" + e);
				} finally {
	...
	private String getSaveFilePath(String path) {
		...
		StaticLog.error(PathUtil.getStaticPath() + path);
		if (!dir.exists()) {
			try {
				boolean isSuccessMakeDir = dir.mkdirs();
				if (!isSuccessMakeDir) {
					StaticLog.error("文件创建失败:" + PathUtil.getStaticPath() + path);
				}
			} catch (Exception e) {
				StaticLog.error("目录创建失败" + PathUtil.getStaticPath() + path);
				return "";
			}
		}
		return path;
